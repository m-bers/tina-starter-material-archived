...
export const Hero = ({ hero }) => {
  ...
  return (
    <Paper className={classes.hero} style={{ backgroundImage: `url(${hero.image})` }}>
      {/* Increase the priority of the hero background image */}
      {<img style={{ display: 'none' }} src={hero.image} /* alt={post.imageText} */ />}
      ...
      <Grid container>
        <Grid item md={6}>
          <div className={classes.heroContent}>
            ...
            {hero.ctas && (<>{Object.keys(hero.ctas).map(key => {
              /* if (hero.ctas[key].primary) {} */
                return (
                  <Button className={classes.heroButton} variant="outlined"
                    /* primary={hero.ctas[key].primary} */
                    href={hero.ctas[key].link}
                    color="inherit"
                  >
                    {hero.ctas[key].label}
